...
ProtocolTest = TestCase("ProtocolTest");

ProtocolTest.prototype.send = function (msg) {
	assertEquals("Send command is invalid", msg, this._mockConnection.sended);
}
...
ProtocolTest.prototype.setUp = function (){
...
ProtocolTest.prototype.testChallenger = function (){
	var chl = "29409134351025259292";
	var digest = this._msn.challenger(chl);
	assertEquals("d0c1178c689350104350d99f8c36ed9c", digest);
}

ProtocolTest.prototype.testSession = function () {
	...
	assertEquals("Invalid host redefinition", "207.46.106.118",this._mockConnection.host);
	assertEquals("Invalid port redefinition", "1863",this._mockConnection.port);
	...
	assertFalse("User not logged, ConnectionListener::onLogged shoudn't be called", this._mockClient.logged);
	...
	assertNotNull('Group is null and should be setted as Sifth', this._mockClient.group);
	assertEquals('Invalid group id', '0', this._mockClient.group.group_id);
	assertEquals('Invalid group name', 'Sifth', this._mockClient.group.name);
	this.receive("LSG 1 Jedis\r\n");
	assertNotNull('Group is null and should be setted as Jedis', this._mockClient.group);
	assertEquals('Invalid group id', '1', this._mockClient.group.group_id);
	assertEquals('Invalid group name', 'Jedis', this._mockClient.group.name);
	
	//Add Emperor as a contact
	assertEquals(this._mockClient.contact, null);
	this.receive("LST emperor@empire.com Emperor 13 0\r\n");
	assertNotNull('Contact is null and should be setted as emperor@empire.com', this._mockClient.contact);
	...
	assertNotNull('Contact is null and should be setted as luke@rebels.org', this._mockClient.contact);
	assertEquals(this._mockClient.contact.user, 'luke@rebels.org');
	assertEquals(this._mockClient.contact.nick, 'Luke');
	assertEquals(this._mockClient.contact.lists, '3'); //Luke dosen't have Vader in their list! Bastard!!
	assertEquals(this._mockClient.contact.groups, '1');
